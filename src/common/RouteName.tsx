//auth screen
export const LOGIN_SCREEN = "[LOGIN SCREEN] LOGIN";
export const REGISTER_SCREEN = "[REGISTER SCREEN REGISTER";
//main screen
export const MAIN_SCREEN = "[MAIN SCREEN] MAIN";
//chat screen
export const CHAT_LIST_SCREEN = "[CHAT SCREEN] CHAT LIST";
export const CHAT_DETAIL_SCREEN = "[CHAT SCREEN] CHAT DETAIL";
//profile
export const PROFILE_INFO_SCREEN = "[PROFILE SCREEN] PROFILE INFO";
export const PROFILE_MODAL_SCREEN = "[PROFILE SCREEN] PROFILE MODAL";
export const POST_CREATE_SCREEN = "[POST SCREEN] POST CREATE";

//title for route name
export const PROFILE_INFO_TITLE = "Profile";
export const PROFILE_MODAL_TITLE = "";
export const POST_CREATE_TITLE = "Create Post";
export const CHAT_LIST_TITLE = "";
export const CHAT_DETAIL_TITLE = "";